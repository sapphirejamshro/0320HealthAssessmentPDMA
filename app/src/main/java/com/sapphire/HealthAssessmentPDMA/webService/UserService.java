...
public class UserService {
    ...
    public void getUsersHavingAssessments(Integer userId,String userType, final VolleyCallback volleyCallback){
    ...
    public void checkAssessmentAllowed(Integer userId,final VolleyCallback volleyCallback){
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("public_user_id",userId);
        } catch (JSONException e) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST,
                RESTApiURLs.CHECT_ASSESSMENT_ALLOWED, jsonObject,
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        volleyCallback.onSuccess(response);
        ...
        jsonObjectRequest.setRetryPolicy(new DefaultRetryPolicy(15000, DefaultRetryPolicy.DEFAULT_MAX_RETRIES, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        requestHandler.addToRequestQueue(jsonObjectRequest,"checkAssessmentAllowed");
    }
}
